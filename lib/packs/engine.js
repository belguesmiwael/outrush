...
export async function generatePackSuggestions(admin) {
  ...
  for (const dormant of dormants) {
    const scored = heroes
      .map((hero) => ({ hero, score: compatScore(hero, dormant, catById, weights) }))
      .sort((a, b) => b.score - a.score);
    // seuil souple ; si rien ne passe (catalogue sans catégories), on garde au
    // moins le meilleur héros disponible → il y a TOUJOURS de quoi composer un lot
    let ranked = scored.filter((r) => r.score >= 0.3).slice(0, 3);
    if (!ranked.length && scored.length) ranked = [scored[0]];

    for (const { hero, score } of ranked) {
      rows.push({
        hero_id: hero.id,
        dormant_ids: [dormant.id],
        compat_score: score,
        margin_sim: simulatePack(hero, [dormant]),
        status: 'proposed',
      });
    }
  }
...
export async function classifyStock(admin) {
  ...
  return { classified: updated, heroes, dormants, proxy: !hasSalesHistory };
}
