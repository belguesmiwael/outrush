...
/**
 * COMPOSITION DES LOTS DU JOUR (manuel) — la même mécanique que le cron 24 h :
 * régénère les suggestions, archive les lots auto de la veille, compose des lots
 * frais avec noms IA. Rotation = rareté ("aujourd'hui seulement").
 */
export async function composeLotsNow() {
  await requireStaff();
  const admin = createAdminClient();
