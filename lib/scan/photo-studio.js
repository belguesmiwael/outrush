...
import { geminiCutout, geminiAvailable } from './gemini-image';
...
export async function studioProcess(inputBuffer, { size = 1200 } = {}) {
  try {
    // Détourage : Gemini d'abord (si clé), sinon remove.bg, sinon photo brute
    let cutout = await geminiCutout(inputBuffer);
    if (!cutout) cutout = await removeBackground(inputBuffer);
