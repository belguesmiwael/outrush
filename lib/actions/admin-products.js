...
export async function enrichGallery(formData) {
  ...
  const { data: product } = await admin
    .from('products')
    .select('id, slug, gtin, images')
    .eq('id', parsed.data.productId)
    .maybeSingle();
  if (!product?.gtin) return { ok: false, error: 'no_gtin' };

  const paths = await fetchOfficialImagesFor(admin, product.gtin, product.slug);
...
export async function generateStudioPreview(formData) {
  ...
  if (out) {
    return { ok: true, dataUrl: `data:image/png;base64,${out.toString('base64')}`, engine: 'gemini' };
  }

  // Gemini indisponible (quota, etc.) → fond signature local (Sharp), sans IA.
  // Toujours un visuel propre à valider, indépendant du quota Gemini.
  try {
    ...
    if (local) {
      return {
        ...
        note: errRef.msg?.includes('Quota')
          ? 'Gemini saturé (quota) — fond signature appliqué localement. Réessayez plus tard pour le détourage IA.'
          : 'Fond signature appliqué localement (sans détourage IA).',
      };
    }
  } catch { /* ignore */ }

  return { ok: false, error: 'gemini_failed', detail: errRef.msg };
}
