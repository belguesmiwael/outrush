...
export async function createFlashSale(payload) {
  ...
  revalidatePath('/');
  revalidatePath('/flash'); revalidateTag('flash');
...
export async function setFlashStatus(formData) {
  ...
  const admin = createAdminClient();
  const patch = { status };
  // Activer maintenant : si le début est dans le futur, on le ramène à maintenant
  if (status === 'live') {
    const { data: sale } = await admin.from('flash_sales').select('starts_at').eq('id', id).maybeSingle();
    if (sale && new Date(sale.starts_at) > new Date()) {
      patch.starts_at = new Date().toISOString();
    }
  }
  await admin.from('flash_sales').update(patch).eq('id', id);
  revalidatePath('/admin/flash');
  revalidatePath('/');
  revalidatePath('/flash'); revalidateTag('flash');
  return { ok: true };
