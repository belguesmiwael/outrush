...
export async function geminiStudio(inputBuffer, mimeType = 'image/jpeg', opts = {}) {
  ...
  try {
    ...
    // Quota par minute dépassé (429) → une pause puis un seul réessai
    if (res.status === 429) {
    ...
    if (!res.ok) {
      ...
      if (opts.errRef) {
        opts.errRef.msg = res.status === 429
          ? 'Quota Gemini dépassé. Attendez ~1 min (limite gratuite ~10/min) ou activez la facturation Google Cloud.'
          : `gemini_${res.status}: ${body.slice(0, 160)}`;
      }
      return null;
    }
