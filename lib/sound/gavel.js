...
/**
 * Cloche douce d'atterrissage du prix « au marteau » (révélation du lot).
 * Plus douce que le marteau : pas de coup de bois, juste une cloche laiton
 * chaude. Opt-in, muet par défaut, respecte prefers-reduced-motion.
 */
export function playReveal({ force = false } = {}) {
  if (!force && !isSoundOn()) return;
  if (prefersReducedMotion()) return;
  const ctx = ac();
  if (!ctx) return;
  ...
  const base = 1318.5; // mi aigu, chaud
  [base, base * 2.0, base * 3.01].forEach((freq, k) => {
    const osc = ctx.createOscillator();
    const g = ctx.createGain();
    osc.type = 'sine';
    osc.frequency.value = freq;
