...
async function fetchOfficialImageUrls(gtin) {
  ...
  const collected = [];
  // Cumule les images de TOUTES les bases (pas seulement la première qui répond)
  for (const base of bases) {
    try {
      ...
      const urls = [
        ...
        p.selected_images?.front?.display?.fr, p.selected_images?.front?.display?.en,
...
/** Recherche des images officielles par NOM de produit dans Open Facts (fallback sans GTIN). */
async function searchImagesByName(query) {
  ...
  for (const base of bases) {
    try {
      const url = `${base}?search_terms=${encodeURIComponent(query)}&json=1&page_size=5&fields=image_front_url,image_url`;
      const res = await fetchWithTimeout(url, { headers: { 'User-Agent': 'OUTRUSH/1.0 (outlet marketplace)' } });
      if (!res.ok) continue;
      const json = await res.json();
      const products = Array.isArray(json?.products) ? json.products : [];
      const urls = products
        .flatMap((p) => [p.image_front_url, p.image_url])
        .map(sanitizeUrl).filter(Boolean);
...
export async function fetchOfficialImagesFor(admin, gtin, slug, name) {
  let urls = await fetchOfficialImageUrls(gtin);
  // Fallback : si le GTIN ne donne rien, cherche par nom de produit
  if (!urls.length && name) urls = await searchImagesByName(name);
...
export async function enrichPhotoScan(scanId) {
  ...
  // Photo réelle du lot : gardée TELLE QUELLE (jamais de détourage / suppression
  // de fond) — juste redimensionnée + optimisée en webp (fidélité + poids réduit).
  let imagePath = null;
  try {
    let finalBuf = buf, finalExt = ext, finalType = mediaType;
    try {
      const sharp = (await import('sharp')).default;
      finalBuf = await sharp(buf).rotate().resize(1400, 1400, { fit: 'inside', withoutEnlargement: true }).webp({ quality: 82 }).toBuffer();
      finalExt = 'webp'; finalType = 'image/webp';
    } catch { /* garde l'original si Sharp indisponible */ }
